...
def run_simulation(seed, warmup=WARMUP_SECONDS, measure_until=MEASURE_UNTIL):
    ...
    PrePressJoinBuffer = simpy.Store(env, capacity=PrePress1Buffer.capacity + PrePress2Buffer.capacity)  # matches PrePress1+PrePress2 cap
    # After both Presses -> shared buffer
    PostPress1_2Buffer = DelayBuffer(env, cap=2, delay=32)
