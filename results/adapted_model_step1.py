...
def run_simulation(seed, warmup=WARMUP_SECONDS, measure_until=MEASURE_UNTIL):
    ...
    PostConveyorBuffer = DelayBuffer(env, cap=6, delay=10)
    PostWashingBuffer = DelayBuffer(env, cap=2, delay=10)
    PrePress1Buffer = DelayBuffer(env, cap=3, delay=32)
    PrePress2Buffer = DelayBuffer(env, cap=4, delay=32)
    PostPress12Buffer = DelayBuffer(env, cap=4, delay=32)
    ...
    PostHanteringBuffer = DelayBuffer(env, cap=1, delay=0)
