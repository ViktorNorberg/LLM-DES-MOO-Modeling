...
"""
# 1. Load the CSV file
df_framework = pd.read_csv("moo_simulation_results4.csv")
...
#facts point
df_facts = pd.read_csv('Factsresults_fixed4.csv')
...
def calculate_hypervolume(points_framework, points_facts):
    ...
    reference_point = np.array([ 40.13042316, -19.76785714])
...
#FOR WIP AND ENERGY CONSUMPTION PER PART

# 1. Load the CSV file
df_framework = pd.read_csv("moo_simulation_results10.csv")

# 2. Extract objectives (assuming last two columns)
# obj1: Minimize (WIP)
# obj2: Minimize (SEC)
points_framework = df_framework.iloc[:, [-3,-2]].values.astype(float)

#points_framework = points_framework[:, [1, 0]]
print(points_framework[:5])


#facts point
df_facts = pd.read_csv('Factsresults_fixed10.csv')
#grab second and third column without column names
points_facts = df_facts.iloc[:, 1:3].apply(pd.to_numeric, errors='coerce').dropna().values.astype(float)
#change places of the two colums
#points_facts = points_facts[:, [1, 0]]
...
def calculate_hypervolume(points_framework, points_facts):
    ...
    ref1 = combined_points[:, 0].max() * 1.1
    ref2 = combined_points[:, 1].max() * 1.1

    reference_point = np.array([ref1, ref2])
    """
    
    reference_point = np.array([40.13042316, 0.5710208])
